...
for (i in 1:17) {
  ...
  fname <- file.path(dir_data, "outputs", "brain_map", "Yeo_17", "single_session",
...
for (i in 1:17) {
  ...
  plot(bMap, idx = i, stat = "se", title = title,
     fname_suffix="idx", fname=fname, zlim=c(-0.02, 0.02))  
}
```
...
for (i in 1:17) {
  label_name <- rownames(prior$template_parc_table)[prior$template_parc_table$Key == i]
  title <- paste0("Yeo 17 Network ", label_name, " (#", i, ") Subject-Level Mean") 
  fname <- file.path(dir_data, "outputs", "brain_map", "Yeo_17", "four_sessions",
                     paste0("posterior_Yeo17_", label_name))
  plot(bMap, idx = i, stat = "mean", title = title,
     fname_suffix="idx", fname=fname,  zlim=c(-0.2, 0.2)) 
}
